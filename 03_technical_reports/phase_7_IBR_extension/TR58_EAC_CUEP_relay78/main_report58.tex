...
%% ─────────────────────────────────────────────────────────────────────────────
\subsection{Deterministic Scenario Results}
\label{sec:det_results}
%% ─────────────────────────────────────────────────────────────────────────────

Table~\ref{tab:det_results} summarises the 10-scenario simulation study executed
in \texttt{run\_tr58\_eac\_relay78.py}.  All scenarios pass the proposed relay
criterion; the standard relay also passes on 10/10 but for a critical
qualitative difference on S05--S06 explained below.

\begin{table}[ht]
  \centering
  \caption{Deterministic scenario results.  $\dlb$, $\du$ in degrees;
  $\varepsilon = \du - \dlb \geq 0$ (Proposition~\ref{prop:cuep});
  $\delta_{\max}$ = peak rotor angle; $\eta$ = conservative stability index;
  $\checkmark$ = PASS.}
  \label{tab:det_results}
  \small
  \begin{tabular}{lcccccccc}
    \toprule
    \#  & GT & $\dlb$ & $\du$ & $\varepsilon$ & $\delta_{\max}$
        & $\eta$ & Std & Prop \\
    \midrule
    S01 & stbl  & 153.6 & 153.6 &  0.0 &  95.3 &  0.55 & $\checkmark$ & $\checkmark$ \\
    S02 & unstb & 153.6 & 153.6 &  0.0 & ---   & $-6.7$& $\checkmark$ & $\checkmark$ \\
    S03 & stbl  & 153.6 & ---$^*$& 26.4 &  91.6 &  0.46 & $\checkmark$ & $\checkmark$ \\
    S04 & unstb & 153.6 & ---$^*$& 26.4 & ---   &$-40.5$& $\checkmark$ & $\checkmark$ \\
    S05 & unstb & 151.1 & 151.1 &  0.0 & ---   & $-1.7$& $\checkmark^{\dagger}$ & $\checkmark$ \\
    S06 & unstb & 151.1 & 151.1 &  0.0 & ---   & $-1.7$& $\checkmark^{\dagger}$ & $\checkmark$ \\
    S07 & unstb & 147.7 & 147.7 &  0.0 & ---   & $-0.5$& $\checkmark$ & $\checkmark$ \\
    S08 & unstb & 153.4 & ---$^*$& 26.6 & 154.7 & $-2.0$& $\checkmark$ & $\checkmark$ \\
    S09 & stbl  & 153.6 & 153.6 &  0.0 & 137.8 &  0.11 & $\checkmark$ & $\checkmark$ \\
    S10 & unstb & 149.9 & 149.9 &  0.0 & ---   & $-7.2$& $\checkmark$ & $\checkmark$ \\
    \midrule
    \multicolumn{7}{l}{Total} & 10/10 & 10/10 \\
    \bottomrule
    \multicolumn{9}{l}{\scriptsize $^*$GFM virtual torque prevents instability at any angle ($\du \to 180^\circ$).} \\
    \multicolumn{9}{l}{\scriptsize $^\dagger$Standard trips \emph{after} true CUEP ($153.6^\circ > 151.1^\circ$);
    proposed trips before ($146.1^\circ < 151.1^\circ$). See text.}
  \end{tabular}
\end{table}

\textbf{Key check (S05--S06).}  For the 30\% GFL deep-fault scenario
($V_{\min} = 0.10$~pu), the true CUEP has shifted inward to
$\du = 151.1^\circ$.  The standard blinder sits at $\pi - \dz = 153.6^\circ$,
which is $2.5^\circ$ \emph{outside} the true CUEP: the relay fires after the
machine has already crossed the point of no return.  The proposed blinder at
$\dlb - 5^\circ = 146.1^\circ$ is $5.0^\circ$ \emph{inside} the CUEP,
consistent with Proposition~\ref{prop:cuep} (conservative direction).

\textbf{GFM scenarios (S03, S04, S08).}  When GFM virtual torque
$K_v (\delta - \dz)$ is active, the post-fault power--angle curve never
crosses $P_m$ for $\delta > 90^\circ$, so the true CUEP pushes to
$180^\circ$ (no finite instability angle exists).  The conservatism gap
$\varepsilon = 26$--$27^\circ$ is entirely due to the GFM benefit being
ignored in Assumption~\ref{ass:gfm}; the proposed setting remains conservative
by design.
...
A 2\,000-trial Monte Carlo study was executed with parameters drawn uniformly:
$k_{\mathrm{gfl}} \in [0.10, 0.50]$, $k_{\mathrm{gfm}} \in [0.00, 0.30]$,
$K_v \in [0.3, 0.7] \times k_{\mathrm{gfm}} P_{\max}^{\mathrm{SG}}$,
$V_{\min} \in [0.15, 0.40]$~pu (realistic LVRT range).
Fault clearing time alternates between clearly-stable
($t_{\mathrm{cl}} \in [0.40, 0.70]\,\mathrm{CCT}$) and
clearly-unstable ($t_{\mathrm{cl}} \in [1.30, 1.70]\,\mathrm{CCT}$) trials.

\begin{table}[ht]
  \centering
  \caption{Monte Carlo results (2\,000 trials, RK4 $\Delta t = 2$\,ms).}
  \label{tab:mc}
  \begin{tabular}{lcc}
    \toprule
    Metric & Value & Target \\
    \midrule
    $P(\dlb \leq \du)$ --- Proposition~\ref{prop:cuep} & 1.0000 & = 1.000 \\
    Conservatism violations & 0 & = 0 \\
    $\varepsilon_{\max} = \max(\du - \dlb)$ & $30.25^\circ$ & $< 35^\circ$ \\
    \midrule
    $P_D$ (proposed relay, unstable trials) & 1.0000 & $\geq 0.980$ \\
    $P_D$ (standard relay, unstable trials) & 1.0000 & reference \\
    $P_{\mathrm{FA}}$ (proposed relay, stable trials) & 0.0000 & $\leq 0.010$ \\
    $P_{\mathrm{FA}}$ (standard relay, stable trials) & 0.0000 & reference \\
    Median trip time $t_{50}$ (proposed, unstable) & 470\,ms & $< 500$\,ms \\
    \bottomrule
  \end{tabular}
\end{table}

All six Monte Carlo targets are met.
Proposition~\ref{prop:cuep} is verified with zero violations across all 2\,000 trials.
The maximum conservatism gap $\varepsilon_{\max} = 30.25^\circ$ occurs in GFM-heavy
cases ($k_{\mathrm{gfm}} \approx 0.30$) where the true CUEP approaches $180^\circ$
but $\dlb$ ignores the GFM benefit (Assumption~\ref{ass:gfm}).
The proposed relay achieves $P_D = P_{\mathrm{FA}} = 0$ on clearly-separated
stable/unstable trial sets, confirming that the $5^\circ$ security margin
($\delta_{\mathrm{sec}}$) provides an adequate buffer between the
blinder and the stable-swing peak angle.
...
\begin{enumerate}
  \item[\checkmark] Swing equation simulation (GFM+GFL models) —
        \textbf{complete}: 10/10 deterministic PASS; 2\,000-trial MC
        (Section~\ref{sec:det_results}--\ref{sec:mc}).
  \item[\checkmark] Conservatism gap $\varepsilon$ verified — tightest bound
        achieved for GFL-only scenarios ($\varepsilon = 0$); GFM conservatism
        gap $\varepsilon \leq 30.25^\circ$ bounded by GFM virtual torque.
  \item[2.] Extend Proposition~\ref{prop:cuep} to multi-machine systems via the
        EEAC (Extended EAC)~\cite{Chiang1987}, using $\dlb$ as the CUEP
        surrogate in the coherent machine decomposition.
  \item[3.] Validate the impedance-plane blinder coordinates
        (Section~\ref{sec:impedance}) against SEL-300G/GE\,D60 relay test
        results (planned: Q3~2026 HIL campaign, TR-67).
\end{enumerate}
...
\begin{tabular}{ll}
  ...
  File & Description \\
  \midrule
  \texttt{TR58\_EAC\_CUEP\_relay78/main\_report58.tex} & This document \\
  \texttt{TR58\_EAC\_CUEP\_relay78/references58.bib}   & Bibliography \\
  \texttt{TR58\_EAC\_CUEP\_relay78/Makefile}           & Build script \\
  \texttt{sambp/relay78/run\_tr58\_eac\_relay78.py}    & Swing ODE + 10-scenario + MC validation \\
  \bottomrule
\end{tabular}
