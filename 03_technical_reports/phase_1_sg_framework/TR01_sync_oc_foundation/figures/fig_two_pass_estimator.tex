...
\node[lbl, above=0.45cm] at ({\tpre+\twin-\ttail/2}, 0)
  {\textcolor{p2C}{\textbf{Pass 2}} \\ \scriptsize tail};
...
\draw[brace, color=p2C]
  ...
  node[midway, above=8pt, lbl, color=p2C]
...
\node[rectangle, rounded corners=3pt, draw=p1C, fill=p1C!10, line width=0.9pt,
      text width=5.5cm, align=left, font=\footnotesize, inner sep=7pt]
...
% Arrow Pass1 → Pass2
\draw[arr, color=gray] (p1box.east) -- ++(0.5, 0)
  node[right, font=\scriptsize, color=gray, align=left]
  {lock $\hat{t}_0,\hat{I}_{sub},\hat{\tau}_{dc},\hat{\phi}_a$};
...
% ---- Caption label ----------------------------------------------------------
\node[font=\small\bfseries, color=black!70] at ({(\tpre+\twin)/2}, {0.9})
