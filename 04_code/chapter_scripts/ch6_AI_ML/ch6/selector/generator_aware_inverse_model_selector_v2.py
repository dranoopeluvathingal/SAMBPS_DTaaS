...
def softmax_like_from_errors(error_dict, sharpness=4.0):
    ...
    exp_scores = np.zeros_like(vals, dtype=float)
    exp_scores[finite_mask] = np.exp(scores[finite_mask] - max_score)
...
def score_source_families(features):
    ...
    s = sum(raw.values())
    if s <= 1e-12:
        probs = {k: 1.0 / len(raw) for k in raw}
    else:
...
# ============================================================
# Family-specific residual models
# ============================================================
...
def model_dfig_like(tau, K1, tau1, f1, phi1, K2, tau2):
    # early oscillatory machine-like burst + slower nonoscillatory tail
    return (
        K1 * np.exp(-tau / tau1) * np.sin(2 * np.pi * f1 * tau + phi1)
        + K2 * np.exp(-tau / tau2)
    )


def model_full_converter_pmsg_like(tau, K, tau_c, phi):
    # smoother control-shaped bounded transient
    return K * (1.0 - np.exp(-tau / (0.25 * tau_c + 1e-6))) * np.exp(-tau / tau_c) * np.sin(phi)


def model_grid_following_inverter_like(tau, K, tau_c):
    # nonoscillatory limited-control response
    return K * np.exp(-tau / tau_c)


def fit_family_model_to_signal(t, y, t_event, family_name):
    mask = t >= t_event
    tau = t[mask] - t_event
    yy = y[mask]
    ...
    try:
        if family_name == 'induction_machine_like':
            ...
            p0 = [np.max(np.abs(y_fit)) + 1e-3, 0.003, 1200.0, 0.0, 0.2*np.max(np.abs(y_fit))+1e-3, 0.010]
            bounds = ([0.0, 1e-4, 100.0, -2*np.pi, 0.0, 1e-4],
                      [5.0, 0.05, 4000.0, 2*np.pi, 5.0, 0.05])
            popt, _ = curve_fit(model_dfig_like, tau_fit, y_fit, p0=p0, bounds=bounds, maxfev=40000)
            y_hat = model_dfig_like(tau_fit, *popt)

        elif family_name == 'full_converter_pmsg_like':
            p0 = [np.max(np.abs(y_fit)) + 1e-3, 0.005, np.sign(np.mean(y_fit) + 1e-9)]
            bounds = ([0.0, 1e-4, -2*np.pi], [5.0, 0.05, 2*np.pi])
            popt, _ = curve_fit(model_full_converter_pmsg_like, tau_fit, y_fit, p0=p0, bounds=bounds, maxfev=30000)
            y_hat = model_full_converter_pmsg_like(tau_fit, *popt)

        elif family_name == 'grid_following_inverter_like':
            p0 = [np.max(np.abs(y_fit)) + 1e-3, 0.004]
            bounds = ([0.0, 1e-4], [5.0, 0.05])
            popt, _ = curve_fit(model_grid_following_inverter_like, tau_fit, np.abs(y_fit), p0=p0, bounds=bounds, maxfev=30000)
            y_hat_abs = model_grid_following_inverter_like(tau_fit, *popt)
...
def family_residual_scoring(t, ta, tb, tc, t_event):
    families = [
    ...
    for fam in families:
        fit_a = fit_family_model_to_signal(t, ta, t_event, fam)
        fit_b = fit_family_model_to_signal(t, tb, t_event, fam)
        fit_c = fit_family_model_to_signal(t, tc, t_event, fam)
    ...
    residual_probs = softmax_like_from_errors(family_errors, sharpness=5.0)
...
def combine_family_probabilities(heuristic_probs, residual_probs, alpha=0.55):
    ...
    if s <= 1e-12:
        combined = {k: 1.0 / len(combined) for k in combined}
    else:
...
def apply_unknown_mixed_logic(combined_probs, winner, winner_prob, margin, family_errors,
                              prob_threshold=0.42, margin_threshold=0.08):
    # unknown/mixed if confidence is weak or residual discrimination is poor
...
def generator_aware_inverse_model_selector_v2(
    input_csv='Z_Final_Thesis_data/ch6_AI_ML/fault_data.csv',
    output_data_dir='Z_Final_Thesis_data/ch6_generator_aware_selector_v2',
    output_fig_dir='Z_Final_Thesis_figures/ch6_generator_aware_selector_v2',
    f0=50.0
):
    ...
    # Generic fitted transients for feature extraction
    ...
    # Residual scoring
    family_errors, residual_probs, family_models = family_residual_scoring(t, ta_raw, tb_raw, tc_raw, t_event)

    # Combined scoring
    combined_probs, combined_winner, combined_p1, second_family, combined_p2, combined_margin = combine_family_probabilities(
        heuristic_probs, residual_probs, alpha=0.55
    )
    ...
    # Best family model reconstruction for plotting
    ...
    summary_csv = os.path.join(output_data_dir, 'generator_aware_selector_v2_summary.csv')
    ...
    json_path = os.path.join(output_data_dir, 'generator_aware_selector_v2_summary.json')
    ...
    plt.bar(x, rvals, width=w, label='Residual')
    ...
    plt.title('Family-Specific Residual Errors')
    plt.grid(True, axis='y', linestyle=':')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_fig_dir, '05_selector_features.png'), bbox_inches='tight')
    plt.show()

    print("\nSUCCESS: Generator-aware selector v2 completed.")
...
if __name__ == "__main__":
    generator_aware_inverse_model_selector_v2(
        input_csv='Z_Final_Thesis_data/ch6_AI_ML/fault_data.csv'
    )