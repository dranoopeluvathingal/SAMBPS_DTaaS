...
External+CT sat.     & \ding{51} & \ding{51} & conventional & 4.9      & 0.17 & 0.45 & ---\\
...
\multicolumn{8}{l}{\small \ding{51}=trip, \ding{55}=no trip.
  Inrush blocked by H2 after 1 cycle; overexcitation by H5.}\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Healthy & No fault        & A & \ding{55} & \ding{55} & no\_trip & 2.5 & 0.00\\
Healthy & External        & A & \ding{55} & \ding{55} & no\_trip & 2.5 & 0.00\\
...
\begin{remark}
  In the Degraded channel scenarios, the conventional relay trips even for
  healthy-line and external-fault scenarios because residual skew (1.8 samples)
  and jitter (2 samples) create a small but non-zero differential.  This is a
  documented limitation of the conventional 87L relay under non-ideal channels;
  it is logged as a security issue requiring Stage~2 SAMBP discrimination
...
\begin{enumerate}
  ...
  \item Full validation: 7/7 87T events and 12/12 87L scenarios correctly
