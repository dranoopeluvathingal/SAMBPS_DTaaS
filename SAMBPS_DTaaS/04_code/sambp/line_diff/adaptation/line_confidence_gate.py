...
class LineConfidenceGateConfig:
    ...
    # Magnitude override: if I_fund >= this threshold AND conventional trip,
    # bypass the Stage-2 veto regardless of f_int.
    # Discriminant: external+HIF → I_fund small; internal+HIF → I_fund large.
    # 0.30 pu is above the sync-error / channel-skew range (< 0.15 pu) yet
    # reachable by a genuine fault at α ≥ 0.20 on a 1.5 pu nominal fault line.
    veto_override_I_thresh: float = 0.30
...
def evaluate_87L_confidence_gate(
    ...
    # Magnitude override: if the estimated fundamental differential is large
    # enough, the fault is almost certainly internal — do not let low f_int
    # (which can result from HIF signal compression) suppress the trip.
    magnitude_override = (
    ...
    # The veto is bypassed when magnitude_override is active.
    ...
    if confirmed:
        ...
        reason     = (f"model veto: f_int={f_int:.3f} < {gate_cfg.f_int_thresh:.3f}, "
                      f"κ_n={kn:.1f}, I_fund={zone_state.I_fund:.3f}")
