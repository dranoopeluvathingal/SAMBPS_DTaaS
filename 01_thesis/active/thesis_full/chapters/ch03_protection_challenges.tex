...
in Chapters~\ref{ch:line_diff}--\ref{ch:centralised}.
...
fault point (typically $\approx 1\angle 0°$~pu). Because
...
\begin{figure}[htbp]
  \centering
  \input{figures/ch3_sympathetic_tripping.tex}
...
Let $\phasor{I}_B = I_B e^{\jj(\phi_A + 90°)}$ (purely reactive, leading).
Then:
\begin{align}
  Z_\mathrm{measured} &= \alpha Z_L + Z_F\left(1 + \frac{I_B}{I_A}e^{\jj 90°}\right)
  \nonumber \\
...
\begin{figure}[htbp]
  \centering
  \input{figures/ch3_87l_failure.tex}
...
\midrule
F1 & ITOC (51) & $I_\mathrm{fault} < I_\mathrm{pickup}$ & Missed trip & Ch~\ref{ch:centralised} \\
F2 & ITOC (51) & Low $M$, large $dt/dM$ & Coordination loss & Ch~\ref{ch:centralised} \\
...
\resultbox[Monte Carlo Sensitivity Study — to be computed from SAMBP simulation]{%
The following simulation results are pending computation from the
SAMBP simulation platform (SAMBP TR-37). Expected outputs:
\begin{itemize}
\item Figure: OC pickup probability vs.\ $\kibr$ for fleet compositions
      with IBR penetration 0--100\%.
\item Figure: 87L sensitivity (ROC curve) vs.\ $k_m$ for fault resistance
      $R_F \in [0, 50]\,\Omega$.
\item Table: Distance relay Zone-1 coverage (\%) vs.\ $\kibr$ and
      line loading (\% nominal).
\item Table: Sympathetic trip rate (\%) vs.\ bus coupling impedance.
\end{itemize}
These results will quantify the severity of each failure mode and set
the performance targets for Chapters~\ref{ch:line_diff}--\ref{ch:centralised}.}
...
protection algorithms developed in Chapters~\ref{ch:line_diff}--\ref{ch:centralised}.
