% fig_triptime_placeholder.tex
% PLACEHOLDER: Operate time vs Rf for fixed and adaptive TMS
% To be replaced with actual simulation data
\begin{tikzpicture}
\begin{axis}[
  ...
  % Improvement markers
  \draw[<->,gray!55,thick]
    (axis cs:75,2.0) -- (axis cs:75,4.8)
    node[midway,right,font=\tiny,gray!55]{53\%};
  \draw[<->,gray!55,thick]
    (axis cs:90,3.9) -- (axis cs:90,7.4)
    node[midway,right,font=\tiny,gray!55]{51\%};

  % Simulation result markers
  \node[font=\tiny\bfseries,blue!70,align=center]
    at (axis cs:72,5.8) {$5.22\,\mathrm{s}$};
  \node[font=\tiny\bfseries,red!70,align=center]
    at (axis cs:72,2.0) {$2.48\,\mathrm{s}$};
